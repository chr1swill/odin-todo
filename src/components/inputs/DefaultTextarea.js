...
export function DefaultInputComponent(placeholder, required = false) {
    ...
    return (
        ...
        <textarea name="${name}" class="bg-primary py-2 px-3 text-text font-bold text-base border-none rounded-md max-w-[100%] h-auto focus:border-accent" ${required ? "required" : ""}>
            ${placeholder}
        </textarea>`
    )
}
